...
from config import prefixPathCOCO, prefixPathVOC2007
...
def get_graph_and_word_file(args, labels):
    ...
    if args.dataset == 'COCO2014':
        WordFilePath = '/hy-tmp/coco2014/vectors.npy'
        # WordFilePath = 'E:/Datasets/coco2014/vectors.npy'
    elif args.dataset == 'VOC2007':
        WordFilePath = 'E:/Datasets/PASCAL_VOC2007/VOCdevkit/VOC2007/voc07_vector.npy'
...
def get_data_loader(rank, args):
    ...
    if args.dataset == 'COCO2014':
        ...
        train_set = Voc07Dataset('train', train_dir, train_anno, train_label, input_transform=train_data_transform)
        test_set = Voc07Dataset('val', test_dir, test_anno, test_label, input_transform=test_data_transform)
    else:
        print('%s Dataset Not Found' % args.dataset)
        exit(1)
    train_ddp_sampler = DistributedSampler(train_set, num_replicas=args.world_size, rank=rank, shuffle=True,
                                           drop_last=True)
    test_ddp_sampler = DistributedSampler(test_set, num_replicas=args.world_size, rank=rank, shuffle=False,
                                          drop_last=False)
